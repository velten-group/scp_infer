# This file is based on code from https://github.com/PayamDiba/SERGIO
# Original code licensed under the GNU General Public License v3.0 (GPL-3.0)
...
class sergio (object):
    ...
    noise_type, decays, dynamics = False, sampling_state = 10, half_responses = None, tol = 1e-3,\
    ...
    splice_ratio = 4, dt_splice = 0.01, migration_rate = None):
        ...
        half_responses: a dictionary with gene IDs as keys and half-responses as values, if None, half-responses are calculated
        ...
        self.half_responses_ = half_responses
        self.tol_ = tol
    ...
    def calculate_half_response_(self, level):
        ...
        and current layer
        """
        half_responses = self.half_responses_
        get_half = not half_responses is None

        currGenes = self.level2verts_[level]

        for g in currGenes: # g is list of all bins for a single gene
            c = 0
            if g[0].Type == 'T':    # T is for target?
                for interTuple in self.graph_[g[0].ID]['params']:
                    ...
                    if get_half:
                        self.graph_[g[0].ID]['params'][c] = (self.graph_[g[0].ID]['params'][c][0], self.graph_[g[0].ID]['params'][c][1], self.graph_[g[0].ID]['params'][c][2], half_responses[g[0].ID])
                    else:
